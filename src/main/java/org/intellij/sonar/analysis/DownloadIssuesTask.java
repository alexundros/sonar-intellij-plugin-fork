...
public class DownloadIssuesTask implements Runnable {
  ...
  private void tryDownloadingIssues(SonarServer sonarServer, String resourceKey) {
    ...
    try {
      issues = sonarServer.getAllIssuesFor(resourceKey, sonarServerConfig.getOrganization(), enrichedSettings);
  ...
  private static class DownloadIssuesTaskBuilder {
    ...
    DownloadIssuesTaskBuilder buildFrom(
      ...
      if (processing) {
        initSettings(enrichedSettings);
      }
      if (processing) {
        checkNotNullServerName();
      }
      if (processing) {
        initSonarServerConfig();
      }
      if (processing) {
        buildDownloadIssuesTask(project, enrichedSettings, psiFiles);
      }
      return this;
    }
    ...
    private void initSonarServerConfig() {
      final Optional<SonarServerConfig> maybeServer = SonarServers.get(serverName);
      if (!maybeServer.isPresent()) {
        processing = false;
      } else {
        sonarServerConfig = maybeServer.get();
      }
    }

    private void buildDownloadIssuesTask(Project project, SonarQubeInspectionContext.EnrichedSettings enrichedSettings,
        List<PsiFile> psiFiles) {
