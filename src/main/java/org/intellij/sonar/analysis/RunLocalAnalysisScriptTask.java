...
public class RunLocalAnalysisScriptTask implements Runnable {
  ...
  private static class RunLocalAnalysisScriptTaskFactory {
    ...
    private TemplateProcessor pathToReportTemplateProcessor;
    ...
    RunLocalAnalysisScriptTask create(
      ...
      initPathToReportTemplateProcessor();
      ...
      final String pathToReport = pathToReportTemplateProcessor.process();
    ...
    private void initPathToReportTemplateProcessor() {
      String pathToReportTemplate = localAnalysisScript.getPathToSonarReport();
      pathToReportTemplateProcessor = TemplateProcessor.of(pathToReportTemplate)
          .withProject(enrichedSettings.project)
          .withModule(enrichedSettings.module);
    }
    ...
    private void addServerConfigurationForProcessing() {
      ...
      if (serverConfiguration.isPresent()) {
        ...
        pathToReportTemplateProcessor.withSonarServerConfiguration(serverConfiguration.get());
      }
    }
    ...
    private void addWorkingDirForProcessing() {
      ...
      pathToReportTemplateProcessor.withWorkingDir(workingDir);
    }
  }
  ...
  private void execute() {
    ...
    streamProcessOutputToConsole(indicator, process);
  ...
  private void streamProcessOutputToConsole(ProgressIndicator indicator, Process process) {
  ...
  private void readIssuesFromReport() {
    sonarConsole.info("Reading issues from " + this.pathToReport);
    String sonarReportContent;
    try {
      sonarReportContent = Files.toString(new File(pathToReport), Charsets.UTF_8);
    } catch (IOException e) {
  ...
  private void createIndexFrom(SonarReport sonarReport) {
    ...
    if (sonarReport.getIssues().isEmpty()) {
      return;
    }
